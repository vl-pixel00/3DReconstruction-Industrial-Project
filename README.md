...
This repository implements a production-scale 3D reconstruction pipeline developed for a university industrial project coursework in collaboration with an industry partner. The framework integrates modern AI technologies with traditional 3D graphics tools to create an automated workflow from text descriptions to multi-view datasets for Gaussian Splatting reconstruction and game development.
...
This part of the project automates creating high-quality training datasets for 3D Gaussian Splatting by systematically capturing multiple calibrated views of 3D objects. Rather than manually positioning cameras and rendering individual frames, this tool streamlines the entire process through an automated Blender pipeline.
...
***This code is shared for educational purposes only. Please do not redistribute or reuse without permission.***